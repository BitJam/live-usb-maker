...
ORDERED_CMDS="$ORDERED_CMDS uuids cheats cheats-syslinux cheats-grub install"
    ALL_CMDS="sizes all defaults $ORDERED_CMDS"
...
Notes:
  - short options stack. Example: fixme
...
main() {
    ...
    case $percent_size in
        [1-9]|[0-9][0-9]|100) ;;
        *) fatal $"Bad size format '%s'.  Should be between %s and %s inclusive." "$(pqh $percent_size)" "1%" "100%"
    esac
    ...
    if [ ${#TARGET} -gt 0 ]; then
        ...
        msg "Using target device %s" "$(pq $TARGET)"
    fi
    ...
    case $from in
        file=*)
            mount_iso_file "$from_val" "$ISO_DIR"
            check_md5 "$from_val"
            mk_from_label $"from file %s" "$from_val"
            ;;

        dev=*)
            mount_device "$from_val" "$ISO_DIR"
            mk_from_label $"copy device %s" "$from_val"
            ;;

        clone=*)
            # First see if we were given a block device to clone
            local from_dev=$(expand_device "$from_val")
            ...
            elif test -d "$from_val"; then
                mk_from_label $"by cloning directory %s" "$from_val"
                clone_directory "$from_val" "$ISO_DIR"

            else
                fatal "Can only clone devices and directories, not '%s" "$from_val"
            fi
            ;;
        *)
            fatal "Internal error.  Bad from variable '%s" "$from"
            ;;
    esac

    show_distro_version "$ISO_DIR" "$from_val"
    ...
    local cheats
    case $CHEATS in
        "") yes_NO "Customize language and timezone?"  && cheats_menus cheats ;;
    off|no) ;;
    yes|on) cheats_menus cheats ;;
         *) cheats=$CHEATS     ;;
    esac

    #--- Okay, now we are ready for final checks and to do the work
    # We start logging right after we've expanded the target device
    start_log "$orig_args" "$CMDS" "$from" "$target_dev" "$cheats"
    ...
    local total_size=$(parted $target_dev unit MiB print | sed -rn "s/^Disk.*: ([0-9]+)MiB$/\1/p")
    local mib_size=$((total_size * percent_size / 100 - 1))
    local ext_size=$((mib_size - FAT_SIZE))
    local total_extra=$((total_size - mib_size))
    ...
    usb_stats $"entire drive"    "$total_size" "$mib_size"  "$total_extra" \
              $"main partition"  "$ext_size" "$ext_needed"  "$ext_extra" \
    ...
    local final_q=$(quest "%s %s %s$nc_co\n$quest_co%s" $"Ready to make live-usb on device" \
        "$(pqq ${target_dev##*/})" "$from_label" $"Shall we begin?")
...
check_from() {
    ...
    local from_dev=$(expand_dev "$cmd_from")
    if [ ${#from_dev} -gt 0 ]; then
...
#------------------------------------------------------------------------------
start_log() {
    local args=$1 cmds=${2# }  iso=$3  dev=$4  cheats=$5
